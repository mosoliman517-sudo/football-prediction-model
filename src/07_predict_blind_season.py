...
# project runs. This script is the honest version: it knows real
# history only through 2024-25. Every team's starting Elo is seeded
# from their REAL rating, blended across their last 3 seasons
# (weighted 50/30/20 toward the most recent) rather than just their
# single most recent season -- a real bad (or fluke-great) season
# shouldn't define a team's whole starting point on its own. From
# there, every 2025-26
# match is predicted from the model's own prior predictions fed
# forward -- Elo/form/goals/shots/half-time/H2H all update on
# predictions, never on the real result.
#
...
# from that one draw. So this runs the whole season many times with
# different random draws and averages the results (a proper Monte
# Carlo), which is what actual season-simulation tools do instead of
# trusting one run.
#
...
# ---------------------------------------------------------------------
# Load + train every regressor on real data only, ONCE -- reused
# across every simulated run below.
# ---------------------------------------------------------------------
...
def run_one_simulation(seed):
    """
    Runs the entire blind season once, start to finish, with its own
    fresh state and its own random draws -- completely independent of
    ...
    # ---- fresh state, seeded from real history (2014-15 - 2024-25) ----
    ...
    # team starts this simulation at their true end-of-2024-25 rating,
    # not a flat 1500 -- exactly mirroring real final standing, same
    # as every other season transition already reverts 33% toward the
    # mean rather than wiping the slate clean.
    home_elo = {}
    away_elo = {}
...
# ---------------------------------------------------------------------
# Run the season N times, average each team's Points/GF/GA across runs
# ---------------------------------------------------------------------

print(f"Simulating the season {N_SIMULATIONS} times (different random draws each run)...")
...
print()
print("=" * 100)
print(f"PREDICTED TABLE (averaged across {N_SIMULATIONS} blind simulations)".center(100))
print("=" * 100)
...
print()
print("=" * 100)
print("POSITION COMPARISON (sorted by real final standing)".center(100))
print("=" * 100)
print(comparison_df.to_string(index=False))

mean_abs_error = comparison_df["Position Diff"].abs().mean()
print(f"\nMean absolute position error (averaged over {N_SIMULATIONS} runs): {mean_abs_error:.1f} places")
print("(single-run blind version scored 6.1 places; 06's non-blind version scored 2.8)")
...
render_table(
    axes[0], predicted_table,
    f"Predicted Table (avg of {N_SIMULATIONS} blind runs)",
